...
| Training | Fine-tune method | QLoRA (4-bit NF4, LoRA adapters) | Required by the brief; at 0.6B a full data-efficiency sweep costs minutes per point. |
| Training | Framework | Unsloth (wrapping TRL `SFTTrainer`) | ~2x faster and ~70% less VRAM, and its `save_pretrained_merged` gives a Hub-ready 16-bit model in one call. |
...
| Compute | Training + eval host | Modal (A10G 24GB, serverless) | Mac has no CUDA; Modal runs are defined in code, so the whole N-sweep is one reproducible script rather than a babysat box. |
...
| Eval | Tuned-model serving | vLLM OpenAI-compatible server on Modal | Serves the HF checkpoint behind the interface `OpenAICompatibleProvider` already speaks — zero new provider code. |
...
| Fine-tune framework | Raw TRL + PEFT | More control we do not need; costs VRAM headroom and setup time. |
...
| Compute | Local MLX / MPS | No bitsandbytes 4-bit on Metal, so it is not QLoRA, and results would not transfer. |
...
| Tuned-model serving | `transformers` in-process in `eval.py` | Requires the grader to own a GPU; also slow enough that a 5-checkpoint sweep becomes painful. |
| Tuned-model serving | HF Inference Endpoints | Extra paid always-on surface for something Modal already covers on demand. |
